...
class VLBertEmbeddings(modeling_bert.BertEmbeddings):
    ...
    def forward(self, region_features, position_features,
        ...
        #print("token_type_ids",token_type_ids.size(), "position_ids", position_ids)
...
class Generator(modeling_bert.BertPreTrainedModel):
    def __init__(self, config):
        ...
        #self.classifier_length = nn.Linear(768, 4)
        self.classifier_length = nn.Linear(768, 1)
    ...
    def forward(self, region_features, position_features,
        ...
        attention_mask[:, :, 100, 101:] = -10000.0
        attention_mask[:, :, 101:, 100] = -10000.0

        hidden_states = self.encoder(embeddings, attention_mask, self.head_mask)[0]
        hidden_states_length = hidden_states[:, 100, :]

        return self.classifier(hidden_states), self.classifier_length(hidden_states_length)
