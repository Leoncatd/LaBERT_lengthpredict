...
def inference(generator, data_loader, device):
    ...
    for iteration, batch in tqdm(enumerate(data_loader, 0), total=len(data_loader)):
        ...
        with torch.no_grad():
            ...
            length_set = torch.zeros(B,1).to('cuda:0')
            ...
            for l, (low, high) in enumerate(config.boundaries, 1):
                ...
                length_set = torch.cat([length_set, length_score[:,l-1:l]], 1)
        ...
        level_prdict = torch.max(length_set, 1)[1]

        for level, preds_per_level in enumerate(pred_list, 1):
            for batch_id, image_id in enumerate(image_ids):
                pred_per_level = tokenizer.decode(preds_per_level[batch_id][1:], end_flags=[EOS])
                pred_per_level = re.sub(r'\b(\w+)( \1\b)+', r'\1', pred_per_level)
                pred_per_level = " ".join(pred_per_level.split(' ')[1:])
                pred_dict[str(level)][str(image_id)] = [{'caption': pred_per_level}]


        for batch_id, image_id in enumerate(image_ids):
            level_id=str(level_prdict[batch_id].data.cpu().numpy())
            level_pred_dict[str(image_id)] = {'level': level_id, 'caption' : pred_dict[level_id][str(image_id)]}
    ...
    return pred_dict, level_pred_dict
...
if __name__ == "__main__":
    ...
    pred_dict, level_pred_dict = inference(generator, data_loader, device)
    ...
    logger.info(f"Saving results to {save_dir}/caption_results_level.json")
    with open(os.path.join(save_dir, 'caption_results_level.json'), 'w') as f:
        json.dump(level_pred_dict, f)
