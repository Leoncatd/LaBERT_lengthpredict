...
from utils.tokenizer import EOS, MASK, PAD, tokenizer, LENGTH
...
class COCOCaptionDataset(Dataset):
    ...
    def _get_item_train(self, index):
        sample = self.samples[index] #Sample(caption=[1037,11389,4562,3564,1999,1037,2380,2894,1012], image_id='COCO_train2014_000000035123')
        input_token_id = sample.caption

        length = len(input_token_id) #<class 'list'>: [1037, 11389, 4562, 3564, 1999, 1037, 2380, 2894, 1012]
        flag = False
        ...
        image_id = torch.tensor(int(sample.image_id[-6:]), dtype=torch.long)
        length_file = json.load(open("/home/dingning/workspace/LaBERT_lengthpredict/id2length_train_aver.json"))
        gt_maxlength = length_file[str(f'{int(image_id):06d}')]
        for i, (l, h) in enumerate(self.boundaries, 1):
            if l <= gt_maxlength <= h:
                gt_maxlength = i

        gt_maxlength = torch.tensor(gt_maxlength, dtype=torch.long)

        return token_type_id, input_token_id, masked_token_id, \
               region_feature, region_class, region_spatial, gt_maxlength,image_id
...
def collate_fn_train(batch):
    ...
    region_feature = torch.stack(batch[3], dim=0) #(3,100,2048)
    region_class = torch.stack(batch[4], dim=0)
    region_spatial = torch.stack(batch[5], dim=0)

    gt_maxlength = torch.stack(batch[6], dim=0)
    image_id = torch.stack(batch[7], dim=0)

    if token_type_id.size() != input_token_id.size():
        print("token_type_id",token_type_id.size(), 'input_token_id',input_token_id.size())
        print(batch[0][0].size(),batch[0][1].size(), batch[0][2].size())
        print(batch[1][0].size(), batch[1][1].size(), batch[1][2].size())
    return token_type_id, input_token_id, masked_token_id, \
           region_feature, region_class, region_spatial, gt_maxlength, image_id
